...
.view{padding:28px 32px 80px;max-width:1200px}
...
.stat .k{font-family:var(--ff-body);font-size:11px;font-weight:500;letter-spacing:1.4px;
  text-transform:uppercase;color:var(--muted-soft)}
.stat .v{font-family:var(--ff-display);font-size:31px;font-weight:600;line-height:1.1;
  letter-spacing:-.6px;font-variant-numeric:tabular-nums;color:var(--ink)}
.stat .v.sm{font-size:25px;letter-spacing:-.4px}
...
.tw{overflow-x:auto;border:1px solid var(--hairline);border-radius:12px;background:var(--surface)}
...
td{padding:9px 14px;border-bottom:1px solid var(--hairline-soft);white-space:nowrap;color:var(--ink2)}
...
.three{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;align-items:start}
...
function render(){
  ...
  const counts={trx:R.tx,biaya:R.ex.length,gaji:R.gajiRoster.filter(r=>r.total).length,shift:R.shiftTot,lead:R.ld.length,kpi:R.kpi.length};
  let html="",lastGrp="";
  ...
  document.getElementById("views").innerHTML=`<section class="view">${({
    dash:vDash,omzet:vOmzet,target:vTarget,trx:vTrx,biaya:vBiaya,
...
function vBiaya(R){
  ...
  <div class="tw"><table><thead><tr><th>Tgl</th><th>Deskripsi</th><th>Jenis</th><th>Kategori</th><th>Vendor</th>
    <th class="n">Nilai</th><th>Skema</th><th class="n">Dibayar</th><th class="n">Sisa</th><th>Status</th><th></th></tr></thead><tbody>
    ${R.ex.length?[...R.ex].sort((a,b)=>(b.tanggal||"")<(a.tanggal||"")?-1:(b.tanggal||"")>(a.tanggal||"")?1:dnum(a.urutan)-dnum(b.urutan)).map(e=>{
      ...
      return `<tr${belum?' style="background:var(--warn-bg)"':""}><td class="mono">${e.tanggal?esc(e.tanggal):'<span class="muted">—</span>'}</td><td>${esc(e.deskripsi)}${e.metode?` <span class="tiny muted">${esc(e.metode)}</span>`:""}</td>
      <td>${e.jenis?esc(e.jenis):'<span class="pill bad">belum</span>'}</td>
      <td class="tiny">${e.kategori?esc(e.kategori):'<span class="muted">belum</span>'}</td>
      <td class="tiny muted">${esc(e.vendor||"—")}</td><td class="n">${rp(n)}</td>
      <td class="tiny">${esc(e.skema||"Lunas")}${e.terminKe?` <span class="muted">#${esc(e.terminKe)}</span>`:""}</td>
      <td class="n">${b?rp(b):'<span class="muted">—</span>'}</td>
      <td class="n"${sisa>0?' style="color:var(--warn)"':""}>${sisa>0?rp(sisa):"lunas"}</td>
